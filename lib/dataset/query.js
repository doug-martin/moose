var comb = require("comb"),
    array = comb.array,
    LiteralString = require("../sql").LiteralString,
    sql = require("../sql").sql,
    ...
    QualifiedIdentifier = sql.QualifiedIndentifier,
...
var Dataset = comb.define(null, {
    instance : {
        ...
        constructor : function() {
            this.__opts = {};
            Dataset.CONDITIONED_JOIN_TYPES.forEach(function(type) {
                if (!this[type + "Join"]) {
            ...
            Dataset.UNCONDITIONED_JOIN_TYPES.forEach(function(type) {
                if (!this[type + "Join"]) {
        ...
        exclude : function() {
            ...
            var clause = (tOpts["having"] ? "having" : "where"), clauseObj = tOpts[clause];
            cond = this._filterExpr.apply(this, cond);
        ...
        from : function(source) {
            ...
            source.forEach(function(s) {
                if (comb.isObject(s)) {
                    for (var i in s) {
                        sources.push(new AliasedExpression(i, s));
                    }

                } else if (comb.isInstanceOf(s, this.constructor)) {
                    sources.push(new AliasedExpression(s, this.datasetAlias(tableAliasNum++)));
                } else if (comb.isString(s)) {
                    var parts = this.splitSymbol(s);
                    var sch = parts[0], table = parts[1], alias = parts[2];
                    if (alias) {
                        s = sch ? new QualifiedIdentifier(sch, table) : new Identifier(table)
                        sources.push(new AliasedExpression(s, alias));
                    } else {
                        sources.push(new Identifier(s));
                    }
                }
            }, this);
        ...
        fromSelf : function(opts) {
            ...
            Object.keys(opts).forEach(function(k) {
                if (NON_SQL_OPTIONS.indexOf(k) >= 0) {
                    fs[k] = nil;
                }
            });
        ...
        grep : function(columns, patterns, opts) {
            ...
            if (opts.hasOwnProperty("allPatterns")) {
                ...
                conds = array.toArray(columns)
                    .map(function(c) {
                        BooleanExpression.fromArgs(["OR"].concat(array.toArray(patterns).map(function(pat) {
                            return StringExpression.like(c, pat, opts);
                        })));
                    });
        ...
        groupAndCount : function() {
            ...
            return group.select.apply(group, columns.concat([Dataset.COUNT_OF_ALL_AS_COUNT]));
        ...
        having : function(cond) {
            cond = comb.argsToArray(arguments);
        ...
        joinTable : function(type, table, expr, options, cb) {
            expr = expr || null,options = options || {};
            var h;
            var usingJoin = comb.isArray(expr) && expr.length;
            ...
            if (comb.isInstanceOf(table, this.constructor)) {
                if (!tableAlias) {
            ...
            if (!expr && !cb) {
                ...
                lastAlias = lastAlias || this.__opts["last_joined_table"] || this.firstSourceAlias();
                if (Expression.isConditionSpecifier(expr)) {
                    var newExpr = [];
                    for (var i in expr) {
                        var val = expr[i];
                        var k = this.qualifiedColumnName(i, tableName);
                        if (comb.isString(val)) {
                            var v = this.qualifiedColumnName(val, lastAlias);
                        } else {
                            v = val;
                        }
                        newExpr.push([k, v]);
                ...
                if (comb.isFunction(cb)) {
                    var expr2 = cb(tableName, lastAlias, this.__opts.join || []);
        ...
        __createBetweenExpression : function(obj, invert) {
            ...
            for (var i in obj) {
                ...
                if (comb.isArray(v) && v.length) {
                    pairs.push(new BooleanExpression("AND", new BooleanExpression("gte", new Identifier(i), v[0]), new BooleanExpression("lte", new Identifier(i), v[1])));
                } else {
        ...
        order : function() {
            var args = comb.argsToArray(arguments);
            return this.mergeOptions({order : array.compact(args).length ? args : null});
        },
        ...
        orderMore : function() {
            var args = comb.argsToArray(arguments);
            if (this.__options.order) {
                args = this.__options.order.concat(args);
            }
        ...
        qualifyTo : function(table) {
            ...
            var h = {};
            array.union(Object.keys(o), Dataset.QUALIFY_KEYS).forEach(function(k) {
        ...
        reverse : function() {
            var args = comb.argsToArray(arguments);
            return this.order(this._invertOrder(args.length ? args : this.__opts.order));
        },
        ...
        select : function() {
            ...
            args.forEach(function(c) {
                if (comb.isHash(c)) {
                    for (var i in c) {
                        select.push(new AliasedExpression(i, c[i]))
                    }
                } else if (comb.isString(c)) {
                    select.push(new Identifier(c));
                } else {
                    select.push(c);
                }
            }, this);
        ...
        selectAppend : function() {
            ...
            if (!currentSelect || !currentSelect.length) {
                currentSelect = [Dataset.WILDCARD];
            }
        ...
        setDefaults : function(hash) {
            var defaults = comb.merge(this.__opts.defaults || {}, hash);
            return this.mergeOptions({defaults : defaults});
        },
        ...
        setOverrides : function(hash) {
            var overrides = comb.merge(this.__opts.overrides || {}, hash);
            return this.mergeOptions({overrides : overrides});
        },
        ...
        "with" :function(name, dataset, opts) {
            ...
            return this.mergeOptions({
                "with" : (this.__opts["with"] || []).concat([comb.merge(opts, {name : name, dataset : dataset})])
            });
        },
        ...
        withRecursive : function(name, nonRecursive, recursive, opts) {
            ...
            var wit = (this.__opts["with"] || []).concat([comb.merge(opts, {recursive : true, name : name, dataset : nonRecursive.union(recursive, {all : opts.unionAll != false, fromSelf : false})})]);
        ...
        _filter : function(clause) {
            var cond = comb.argsToArray(arguments).slice(1);
            cond = cond.length == 1 ? cond[0] : cond
            if (cond.length == 0) {
                return this.mergeOptions();
            } else {
                cond = this._filterExpr(cond);
                var cl = this.__opts[clause];
                if (cl) {
                    cond = new BooleanExpression("AND", cl, cond);
                }
                var opts = {};
                opts[clause] = cond;
                return this.mergeOptions(opts);
        ...
        compoundClone : function(type, dataset, options) {
            var ds = this._compoundFromSelf().mergeOptions({compounds : this.__opts.compounds.concat([type, dataset._compoundFromSelf(), options.all])});
            return options.fromSelf ? ds.fromSelf(options) : ds;
        },
            ...
            if (order) {
                ret = order.map(function(o) {
                    if (comb.isInstanceOf(o, OrderedExpression)) {
                        ...
                        return new OrderedExpression(o);
                    }
                }, this);
            }
            return ret;
        ...
        __filterObject : function(expr, key) {
            ...
            for (var k in expr) {
                ...
                } else if (key && (this.TWO_ARITY_OPERATORS[k.toUpperCase()] || k.match(/between/i))) {
                    key = key.split(",");
                    if (key.length > 1) {
                        opts = [
                            [key, v]
        ...
        getters : {
            isSimpleSelectAll : function() {
                ...
                for (var i in opts) {
                    if (opts[i] != null && NON_SQL_OPTIONS.indexOf(i) == -1) {
                        o[i] = opts[i];
                        count++;
                    }
                }
                f = o.from;
                return count == 1 && f.length == 1 && (comb.isString(f[0]) || comb.isInstanceOf(f[0], AliasedExpression));
            }
        }
    },
