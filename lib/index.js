...
var Moose = comb.singleton(migration, {
    instance : {
         ...
         *                            will be used .
         *
         * @return {Query|null} a query object or null if {@link moose#createConnection} has not been called.
         */
        getConnection : function(autoClose, database) {
            var ret = null;
            if (connectionReady) {
                ret = this.client.getConnection(autoClose, database);
            } else {
                LOGGER.error("Connection has not been initialized");
            }
            return ret;
        },
         ...
         *                            will be used.
         *
         * @return {TransactionQuery} a transaction context or null if {@link moose#createConnection} has not
         *                            been called.
         */
        transaction : function(database) {
            var ret = null;
            if (connectionReady) {
                ret = this.client.transaction(database);
            } else {
                LOGGER.error("Connection has not been initialized");
            }
            return ret;
         ...
         *                            will be used
         *
         * @return {Dataset} a dataset to operate on a particular table or null if {@link moose#createConnection}
         *                  has not been called.
         */
        getDataset : function(tableName, database) {
            ...
            if (connectionReady) {
                if (tableName) {
                    ret = dataset.getDataSet(tableName, this.getConnection(false, database), this.type);
                } else {
                    LOGGER.error("Table name required get getting a dataset");
                }
            } else {
                LOGGER.error("Connection has not been initialized");
            }
         ...
         *                            will be used
         *
         * @returns {comb.Promise|null} a promise that will be called after the SQL execution completes, or null if
         *                              {@link moose#createConnection} has not been called.
         */
        execute : function(sql, database) {
            var promise = null, db;
            if (connectionReady) {
                promise = new Promise();
                db = this.getConnection(true, database);
